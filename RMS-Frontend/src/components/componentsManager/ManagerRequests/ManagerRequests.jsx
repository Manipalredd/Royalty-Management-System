      ...
      try {
        ...
        await Promise.all(
          response.map(async (req) => {
            const artistInfo = await ApiService.getArtistInfo(req.artistId);
            if (artistInfo) {
              artistData[req.artistId] = {
                name: `${artistInfo.firstName} ${artistInfo.lastName}`,
      ...
      {requests.length === 0 ? (
        ...
        requests.map((req, index) => (
          <div key={index} className="bg-gray-700 p-4 rounded-lg shadow-md text-center mb-4">
            <p className="text-lg text-white">Artist: {artistDetails[req.artistId]?.name || "Loading..."} </p>
            <p className="text-lg text-white">Email: {artistDetails[req.artistId]?.email || "Loading..."} </p>
            ...
            <button
              onClick={() => respondToRequest(req.partnershipId, "Accepted")}
              className="bg-green-600 text-white px-4 py-2 rounded-lg mx-2"
            >
              Accept
            </button>
            <button
              onClick={() => respondToRequest(req.partnershipId, "Rejected")}
              className="bg-red-600 text-white px-4 py-2 rounded-lg mx-2"
            >
              Reject
            </button>
          </div>
